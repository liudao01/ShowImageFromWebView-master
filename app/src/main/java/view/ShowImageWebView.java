...
//        });
    ...
    private class MyJavascriptInterface {
        ...
        public void startShowImageActivity(String url) {
            Log.d(TAG, "startShowImageActivity: url = " + url);
            ...
            Log.d(TAG, "startShowImageActivity: 打印几次 = ");
...
//            listImgSrc = getAllImageUrlFromHtml(html);
        }
    }
    ...
    private List<String> getAllImageUrlFormSrcObject(List<String> listImageUrl) {
        ...
        for (String image : listImageUrl) {
            ...
            while (matcher.find()) {
                ...
                lists.add(matcher.group().substring(0, matcher.group().length() - 1));

            }

        }
        for (String cd : lists) {
            if (!listImgSrc.contains(cd)) {
                listImgSrc.add(cd);
            }
        }
        return listImgSrc;
